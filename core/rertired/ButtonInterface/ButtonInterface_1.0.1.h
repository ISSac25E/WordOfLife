...
private:
  ...
  bool _pinTest = false;
  uint32_t _pinTimer = micros(); // micros should return faster then millis()
};
...
bool ButtonInterface::run()
{
  ...
  if (_pinBoot) // small cooldown during boot(to prevent button glitches during brownout)
  {
    if (_pinTest)
    {
      if (micros() - _pinTimer >= ((uint32_t)ButtonInterfaceDebounceDelay_ms * (uint32_t)1000))
      {
  ...
  {
    _buttonState = _pinRead();
    if (micros() - _pinTimer >= ((uint32_t)ButtonInterfaceBootDelay_ms * (uint32_t)1000))
    {
      _pinBoot = true;
      _pinInterval = millis();
