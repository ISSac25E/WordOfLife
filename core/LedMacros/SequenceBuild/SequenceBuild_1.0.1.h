...
public:
  ...
  void setSequence(uint8_t (*)(uint8_t), uint8_t, bool); // set a new sequence and start running it only if it isnt already running. input(sequenceFunct, indexToStartFrom, forceFirstSter(default false/wait for current sequence to finish))
  void setSequence(uint8_t (*)(uint8_t));                // set sequence without running it, will stop whichever sequence is currenlty running. Use "start" to begin sequence
