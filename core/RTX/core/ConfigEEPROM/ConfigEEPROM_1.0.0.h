...
public:
  ...
  bool write();                // returns weather or no the eeprom is currently writing. return true = writeing, false = not writing(standby)
...
{
  if (sectors <= ConfigEEPROM_MaxSector && sectors > 0)
    _sectors = sectors;
  else if (sectors == 0)
    _sectors = 1;
  else
