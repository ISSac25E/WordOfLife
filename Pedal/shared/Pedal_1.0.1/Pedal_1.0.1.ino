/*
  changes:
    - color mode easter-egg included
*/
...
// include par vars in case needed
...
// tally states:
const uint8_t pedalTally = 3; // this is the tally that will show in the led's. 1 - 8 tally input
uint8_t tally_program = 0;
uint8_t tally_preview = 0;

bool colors = false; // if color mode is switched on :)
...
void buttonHandle()
{
  const uint8_t defaultInput = 3;
  const uint8_t secondaryInput = 1;
  static Par_bool holdMacros = false;

  bool pinStates[] =
      {RightPedal, MiddlePedal, LeftPedal};

  static bool colorTriggerValid = false;
  if (pinMacro[1](buttonCommand(B101, pinStates, true))) // state change
  {
    if (pinMacro[1] && !buttonCommand(B010, pinStates, false))
    {
      colorTriggerValid = true;
    }
    if (!pinMacro[1] && colorTriggerValid && !buttonCommand(B010, pinStates, false))
    {
        holdMacros = false;
        holdMacros.change();
        holdMacros = true;
      colors = true;
      colorTriggerValid = false;
    }
  }
  ...
  if (pinMacro[0](buttonCommand(B111, pinStates, false))) // state change
  {
    ...
    else if (!pinMacro[0] && pinMacro[0].prevTriggered())
    {
      ...
      else if (!holdMacros.change())
      {
        holdMacros = false;
      }
    }
  }
  if (!pinMacro[0].triggered() && pinMacro[0].interval() >= 500) // timeout delay, to prevent double clicks
  {
    pinMacro[0].trigger();
  }

  if (pinMacro[2](buttonCommand(B111, pinStates, false))) // state change
  {
    if (pinMacro[2])
    {
    }
...
void ledHandle()
{
  ...
  if (colors)
  {
    ledBuild.setSequence(colorMode, 0, true);
  }
  else if (atem_connected)
  {
...
SB_FUNCT(colorMode, allLedReady())
...
SB_STEP(macro.quadEase(pedal_R, 50, 180);
        macro.quadEase(pedal_G, 50, 180);
        macro.quadEase(pedal_B, 0, 180);
        macro.quadEase(tally_R, 50, 180);
        macro.quadEase(tally_G, 50, 180);
        macro.quadEase(tally_B, 0, 180);)
SB_STEP(macro.quadEase(pedal_R, 0, 180);
        macro.quadEase(pedal_G, 50, 180);
        macro.quadEase(pedal_B, 50, 180);
        macro.quadEase(tally_R, 0, 180);
        macro.quadEase(tally_G, 50, 180);
        macro.quadEase(tally_B, 50, 180);)
SB_STEP(macro.quadEase(pedal_R, 50, 180);
        macro.quadEase(pedal_G, 0, 180);
        macro.quadEase(pedal_B, 50, 180);
        macro.quadEase(tally_R, 50, 180);
        macro.quadEase(tally_G, 0, 180);
        macro.quadEase(tally_B, 50, 180);)
SB_STEP(ledBuild.loop(1);)
SB_END
