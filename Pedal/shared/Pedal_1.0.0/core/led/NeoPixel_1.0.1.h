...
/*
  changes
    - removed all static vars, in case future error might appear
*/
#ifndef NeoPixel_h
#define NeoPixel_h
...
public:
  ...
  void run(bool);               // runs neopixel driver. if pixel values change, driver will write immediately. Inputs (forceWrite(default = false))
...
private:
  ...
  uint32_t _pixelWriteTimer = millis();
...
{
public:
  NeoPixel(NeoPixelDriver &, uint8_t); // give instance of NeoPixelDriver and state which pixel channel is needed(optional). Automatically selects next pixelChannel on list. inputs(NeoPixelDriver instance, pixelChannel(optional))
  operator uint8_t &()
  {
...
{
  bool pixelWriteInterval = (millis() - _pixelWriteTimer >= _writeInterval_ms);
  ...
  {
    _pixelWriteTimer = millis();
    for (uint8_t x = 0; x < _pixelCount; x++)
      _pixelBuffer[x + _pixelCount] = _pixelBuffer[x];
    while (micros() - _frameCoolDownTimer < 100) // wait for frame cooldown, otherwise will lead to a miss write
      ;
